...
CREATE TABLE `clicksign_contracts` (
	...
	`extracted_vins` text,
	`customer_id` int,
	`status` enum('pending','signed','processed','error','ignored') NOT NULL DEFAULT 'pending',
	`envelope_status` varchar(50),
	`envelope_name` varchar(500),
	`raw_payload` text,
...
CREATE TABLE `customers` (
	...
	`email` varchar(500),
	`phone` varchar(255),
	`status` enum('aguardando_embarque','aguardando_li','em_processo','concluido','cancelado') NOT NULL DEFAULT 'aguardando_embarque',
	`tipo_operacao` enum('importacao','exportacao'),
	`clicksign_envelope_id` varchar(100),
	`clicksign_signer_id` varchar(100),
	`data_source` enum('manual','clicksign','agent') NOT NULL DEFAULT 'manual',
...
CREATE INDEX `idx_blv_customer` ON `bl_vehicles` (`customer_id`);--> statement-breakpoint
...
CREATE INDEX `idx_cs_envelope` ON `clicksign_contracts` (`envelope_id`);--> statement-breakpoint
CREATE INDEX `idx_cs_cpf` ON `clicksign_contracts` (`signer_cpf`);--> statement-breakpoint
CREATE INDEX `idx_cs_customer` ON `clicksign_contracts` (`customer_id`);--> statement-breakpoint
...
CREATE INDEX `idx_customer_status` ON `customers` (`status`);--> statement-breakpoint
CREATE INDEX `idx_customer_tipo_op` ON `customers` (`tipo_operacao`);--> statement-breakpoint
CREATE INDEX `idx_customer_clicksign` ON `customers` (`clicksign_envelope_id`);--> statement-breakpoint
